...
fn main() {
    ...
    let comb = Combination::new(n + k + 1, ModInt998244353::modulus() as usize);

    let mut arr = vec![false; 4e5 as usize + 1];
    ...
    for i in 0..(n + k) {
        let Some(rest) = k.checked_sub(need) else {
            break;
        };
        rs += comb.get(rest + i, i);
        if !arr[i] {
            need += 1;
        }
    }
