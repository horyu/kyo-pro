...
fn main() {
    ...
    let comb = Combination::new(n, 1000000007);
    let mut rs = ModInt1000000007::default();
    for (i, a) in aa.into_iter().enumerate() {
        if k - 1 <= i {
            rs += a * comb.get(i, k - 1) as isize;
            // eprintln!("{i} {a}: +{} ", comb.get(i, k - 1));
        }
        if k - 1 <= n.saturating_sub(i + 1) {
            rs -= a * comb.get(n.saturating_sub(i + 1), k - 1) as isize;
            // eprintln!("{i} {a}: -{} ", comb.get(n.saturating_sub(i + 1), k - 1));
        }
    }
    println!("{rs}");
}
